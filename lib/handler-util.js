...
function handleTopPage(req, res) {
  ...
  res.write('<h1>NNチャット エントランス(NNChat Entrance)</h1>');
  res.write('<p>NNチャットにようこそ! 今日はどんなことが話せるかな？</p>');
  res.write('<p><a href="/posts">NNチャットに入る</p>');
  res.write('<p><a href="/logout">ログアウトする</p>');
  res.end();
}
